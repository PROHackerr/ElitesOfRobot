...
from ElitesOfRobot import (dispatcher, STRICT_GBAN, GBAN_DUMP, ERROR_DUMP, SUPPORT_CHAT,
                           OWNER_ID, DEV_USERS, SUDO_USERS, SUPPORT_USERS, WHITELIST_USERS, spamwtc)
...
def enforce_gban(update, context):
    ...
    try:
        restrict_permission = update.effective_chat.get_member(context.bot.id).can_restrict_members
    except Unauthorized:
        return

    if (
        sql.does_chat_gban(update.effective_chat.id)
        and restrict_permission
       ):
