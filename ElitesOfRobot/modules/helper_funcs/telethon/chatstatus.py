from ElitesOfRobot.modules.helper_funcs.telethon import STAFF_USERS, telethon
from ElitesOfRobot import SUDO_USERS
...
async def user_is_ban_protected(user_id: int, message):
    status = False
    if message.is_private or user_id in (STAFF_USERS):
        return True

    async for user in telethon.iter_participants(
...
async def is_bot_admin(chat_id: int):
    status = False
    chizuru = await telethon.get_me()
    async for user in telethon.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if chizuru.id == user.id:
            status = True
            break
    return status
